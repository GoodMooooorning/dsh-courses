...
目标工作区与开关都在 **设置 → 插件 →「普瑞塞斯 · 源石协议 主题」** 卡片中配置（即时生效、持久保存）。
...
> 主题显示/关闭**只由设置页的「主题开关」与「目标工作区」控制**，没有 URL 参数或
> localStorage 强制开关。旧版 `localStorage['ak-force']` 与 `?ak=` 机制已移除
> （之前设置过的话可手动删除 `localStorage.removeItem('ak-force')`，新版本已忽略它）。
