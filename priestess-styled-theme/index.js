...
export const Config = Schema.object({
  ...
  target: Schema.string().default('deepseek_workspace').description('启用主题的工作区文件夹名（会话 cwd 的目录名；可被 URL ?aktarget= 或 localStorage ak-target 覆盖）'),
...
async function readPatch(req) {
  ...
  const types = { enabled: 'boolean', target: 'string' }
